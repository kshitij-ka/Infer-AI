...
`/metrics` requires an admin role JWT, matching this application's RBAC model. A standard Prometheus server cannot perform an interactive login, so a production deployment that wires up a real scraper would issue it a long lived, admin scoped service account token (created through the same `/admin/users` and `/auth/login` flow every other account uses) and configure that token as a static bearer credential in the scrape config, or place the scraper behind a network boundary that reaches `/metrics` through an internal path with its own separate authentication. No such Prometheus server is deployed in this repository; see docs/observability.md for what is and is not included here.

## Database migrations

Schema changes go through Alembic rather than `Base.metadata.create_all` running against a live database, so every change to the `users` or `chat_logs` tables has an explicit, reviewable, ordered migration file instead of the schema being derived implicitly from whatever the models currently look like. `alembic/env.py` reads the database connection string from this application's own `Settings` object (`get_settings().database_url`) rather than a value hardcoded in `alembic.ini`, so migrations always run against whichever database the rest of the application is configured to use, in every environment, without a second place to keep a connection string in sync. `env.py` also explicitly imports both model modules (`app.models.chat_log`, `app.models.user`) before referencing `Base.metadata`, since SQLAlchemy only registers a model on the shared declarative base once its module has actually been imported somewhere; forgetting this import is a real mistake this project already made once, in the integration test tier's setup, where a missing import silently produced an incomplete schema missing the `chat_logs` table (see docs/testing.md).

There is exactly one migration today, `alembic/versions/0001_initial.py`, creating both tables in one file, matching the fact that this is a new application starting from an empty schema rather than one with an evolving history yet. New migrations are written by hand (`alembic revision -m "description"` creates the file, but its `upgrade` and `downgrade` bodies are not autogenerate-verified in this project, so they need the same review a hand written database change would get anywhere else). `docker-compose.yml`'s `api` service runs `alembic upgrade head` on container startup, before `uvicorn` starts, so a fresh database is always brought to the current schema automatically when the stack comes up.
...
## Response caching implementation notes

The chat endpoint caches answers in Redis, keyed by a SHA-256 hash of the normalized (trimmed, lowercased) question text, with a configurable TTL (`CACHE_TTL_SECONDS`, default 300). Caching is scoped to the question text only, not per user, since the answer to a factual question does not depend on who asked it, and this maximizes the cache hit rate across users asking the same thing. Every request still writes a ChatLog row and updates Prometheus metrics regardless of cache status, with a distinct `cache_hit` status value and zero token counts on a hit, so token usage and latency metrics stay meaningful (a cache hit's near zero latency does not get mixed into the same latency bucket as a real LLM call without being distinguishable by status). Fallback answers from a failing LLM are never cached, so a temporary outage does not get remembered as the permanent answer once the LLM recovers.

A security review raised a real, if bounded, risk worth stating plainly: this application sends free form user questions to an LLM and returns whatever the LLM answers with, cached or not. An LLM can be induced through prompt injection to produce output the requester did not ask for and a reader would not expect. Caching does not create this risk, since it exists for every uncached answer too, but caching does widen its blast radius: a single successful injection against a common question is then served to every other user asking the same normalized question until the cache entry expires, up to `CACHE_TTL_SECONDS` (default 300 seconds) later, rather than affecting only the one requester who triggered it. Any system that consumesthis API's `answer` field must treat it as untrusted free text, the same way it would treat any other LLM output, and must not render it as trusted HTML or markup without the same output encoding it would apply to any other user supplied or third party content. This application has no such rendering client in this repository; the requirement falls on whatever does consume the API. If a cached answer is ever found to be a problem, it can be removed from Redis directly with the question's known text, for example `redis-cli DEL chatcache:$(printf '%s' 'the exact normalized question' | sha256sum | cut -d' ' -f1)`, or the entire cache namespace can be cleared with `redis-cli --scan --pattern 'chatcache:*' | xargs -r redis-cli DEL` if the affected question text is not known. There is no admin API endpoint for this, since the assessment's scope did not call for one and a direct Redis command is sufficient for an operator to act on an incident.
